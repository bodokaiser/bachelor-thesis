...
\section{Individual acousto-optic deflectors}
...
behaviour of the single \gls{aod} elements as we want to find out if and how
they differ.
...
For the following experiments we will only leave one \gls{aod} element mounted
...
\Cref{fig:intensity_unpaired}. The color values are normalized in between the
different heatmaps and can be related to the measured voltage from the
photodiode by the colorbar on the right-hand side.

\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\textwidth]{\figuredir{intensity/distribution/unpaired-amplitude.pdf}}
