...
measurements of the frequency resolution and the amplitude resposne. Physical
analysis of the \gls{dds} output \gls{rf} signal is in fact no simple
endeavour as usual operation time scales are of many magnitudes
greater than the signal periodicity. The strategy we used to resolve this
circumstance is depcited in \Cref{fig:signal_window}. The strategy
...
\Cref{fig:signal_window_setup}. In between the oscilloscope and the
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{\figuredir{signal/setup-dds.pdf}}
...
\end{figure}
In \Cref{tab:signal_parameters_dds} we can find an overview of the
...
frequency response spectrum with manageable effort. Alternatively one could
try to perform a mean peak-to-peak measurement through the oscilloscope, which
implementation would have been a big time investment.
\begin{figure}[ht]
  \centering
